...
function CreateBudgetCard(props) {
    ...
    const handleSelectCategoryItemToDelete = (value) => {
        if (value === "") {
            return;
        }
        value = JSON.parse(value);
        setDashboardBudgetCardCategoryItemToDelete(value);
    }
    ...
    function deleteCategoryItem() {
        ...
        fetch("http://localhost:3001/deletepersonalbudgetcategoryitem", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "CSRF-Token": user.csrf
            ...
            body: JSON.stringify({
                userId: user.userId,
                personalBudgetCategoryId: categoryItemToDeleteData.personalBudgetCategoryId,
                personalBudgetCategoryItemId: categoryItemToDeleteData.personalBudgetCategoryItemId
            }),
            credentials: "include"
        })
            .then(res => res.json())
            .then(data => {
                if (data.error) {
                    toast.error("There was an error deleting the category item.");
                }
                if (!data.error && !data.exists) {
                    setDashboardBudgetCardCategoryItemToDeleteData(data);
                    toast.success("Category item successfully deleted.");
                }
            })
    }
                    ...
                    ">
                        ...
                        <div className="flex flex-column mv2">
                            ...
                            </div>
                            {
                                (Array.isArray(categoriesAndItems)) ?
                                    <select className="h2 flex mh3 pl1 input-reset bg-custom-lighter-gray custom-gray border-custom-gray form-line-active b bw1"
                                        onChange={(event) => handleSelectCategoryItemToDelete(event.target.value)}
                                        //value={categoryItemToDeleteData.personalBudgetCategoryItemId !== 0 ? () => {} : ""}
                                    >
                                        <option value="">--</option>
                                        {
                                            categoriesAndItems.map(category => {
